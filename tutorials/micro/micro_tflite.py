...
#  TARGET and BOARD used to build a microTVM firmware for the Arm reference board mps2-an521:
#  https://developer.arm.com/tools-and-software/development-boards/fpga-prototyping-boards/mps2
#  On Zephyr '$ west boards' will list the name of the supported boards, and if the board supports
#  to be ran by default emulated (check board .yaml config file for the field "simulation:"). If
#  the board name doesn't start with prefix "qemu_" then it's necessary to add suffix "-qemu" to
#  tell to microTVM it must use a QEMU transporter to communicate with the harware once it's up and
#  running (instead of flashing the firmware and trying to communicate with it via the serial port).
#
...
# enable printing memory usage statistics for the runtime image
# generated by Zephyr
